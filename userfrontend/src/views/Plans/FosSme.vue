<template>
  <div class="main-container">
    ...
    <div class="sme-header">
      <p class="sme-heading">FOS SME Plans</p>
      <!-- <p class="sme-heading">{{ data.name }}</p> -->
      ...
      <p class="sme-sub-heading">Experience Unrivaled Broadband Connectivity</p>
      <div class="sme-header-button">
        <!-- <div v-for="planItem in data">
          <div class="sme-header-button" v-for="planType in planItem.planTypes" :key="planType.name"> -->
            
            <a @click="gotoSme" class="sme-link">SME Broadband </a>
            <!-- <a href="/plan/fos-sme" class="sme-link">{{ planType.name }}</a> -->
  ...
  export default {
    ...
    setup () {
      ...
      onMounted(async()=>{
        try{
          ...
          // filter plans to get only for SME BROADBAND
            filteredPlan.value = plans.value.filter(plan => plan.planName === 'SME BROADBAND')[0]
           const filteredPlanString = JSON.stringify(plans.value.filter(plan => plan.planName === 'SME BROADBAND'), null, 2)
      ...
      const selectPlan = (planItem) => {
        // const selectedPackageName = packageName.value.innerText
        // const selectedPlanName = planItem.planTypeName
        ...
        const token = sessionStorage.getItem('token')
          ...
          if (token) { 
            if (window.location.href.includes('roi')) {
              ...
              router.push({name: 'SmeForm', query: {selectedPlanId, selectedPlanTypeId, smeOrderId }} )
              
            }else{

              router.push({name: 'SmeForm', query: {selectedPlanId, selectedPlanTypeId }} )
            }
          }else {
      ...
      const gotoXtreme = () =>{
        if (window.location.href.includes('roi')) {
          const roi = route.query.roi
            const plan = 'sme'
          router.push({name: 'FosSmeXtreme', query:{roi, plan}})
        } else {
          router.push({name: 'FosSmeXtreme'})
        }
      }
      ...
      const gotoSme = () =>{
        if (window.location.href.includes('roi')) {
          ...
          router.push({name: 'FosSme'})
        }
      }


      return { router, plans, genericSetUpCharge, gotoXtreme, gotoSme, filteredPlan,
        convertToTitleCase, loading, selectPlan, packageName, packagePrice, packageSpeed, packageSetUpCharge, filteredPlanType, planId}
    }
  }
</script>
